...
Shader "Martin/ImageDistortionAlpha"
{
	Properties
	{
		...
		_DistortionAlpha ("DistortionAlpha", Range(0,1)) = 0
	...
	SubShader 
	{
		...
		////// Shader functions
			...
			float4 frag(Vert2Frag fragIn) : SV_Target
			{
				...
				float4 final = tex2D( _MainTex, fragIn.uv+normal.xy ) * _MainColor;
				final.w *= ((1 - _DistortionAlpha) + (_DistortionAlpha*strength));
                return final;
			}
			ENDCG
		}
