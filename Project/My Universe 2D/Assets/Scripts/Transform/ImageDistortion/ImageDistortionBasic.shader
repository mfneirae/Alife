...
Shader "Martin/ImageDistortionBasic"
{
	Properties
	{
		_MainTex ("MainTexture", 2D) = "white" {}
		_DistortionStrength ("Distortion", Range(0,1)) = 0
		_XTimer ("XTimer", Range(0,01)) = 0
		_YTimer ("YTimer", Range(0,01)) = 0
	}
	SubShader 
	{
		// Subshader Tags
		...
		////// Shader functions
			...
			float4 frag(Vert2Frag fragIn) : SV_Target
			{
				...
				normal.x *= sin(_XTimer);
				normal.y *= sin(_YTimer);

                return tex2D( _MainTex, fragIn.uv+normal.xy );
			}
			ENDCG
		}
	} 
